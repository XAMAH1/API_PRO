...
def personal(connect):
    try:
        command = personalvisit.select()
        ...
        for i in result:
            ...
            for j in currentresult:
                user.append(
                    {"user": {"id": i[0], "fio": i[1], "phone": i[2], "mail": i[3], "brithday": i[4], "passport": i[5]},
                     "purpose": {"purpose": j[6], "dateentry": j[4], "dateexit": j[5], "podraz": j[2],
                                 "fio": j[3], "result": j[8], "answer": j[9], "visit": "group"}})
            return {
                "success": True,
                "message": user
            }
